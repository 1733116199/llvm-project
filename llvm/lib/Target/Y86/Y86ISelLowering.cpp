...
//===----------------------------------------------------------------------===//
//
// This file implements the Y86TargetLowering class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "y86-lower"

static cl::opt<bool>Y86NoLegalImmediate(
  "y86-no-legal-immediate", cl::Hidden,
  cl::desc("Enable non legal immediates (for testing purposes only)"),
  cl::init(false));

Y86TargetLowering::Y86TargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(Y86::SP);
  ...
  if (STI.hasHWMult16()) {
    ...
    setLibcallCallingConv(RTLIB::MUL_I64, CallingConv::Y86_BUILTIN);
  }

  // Several of the runtime library functions use a special calling conv
  setLibcallCallingConv(RTLIB::UDIV_I64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::UREM_I64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::SDIV_I64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::SREM_I64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::ADD_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::SUB_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::MUL_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::DIV_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::OEQ_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::UNE_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::OGE_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::OLT_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::OLE_F64, CallingConv::Y86_BUILTIN);
  setLibcallCallingConv(RTLIB::OGT_F64, CallingConv::Y86_BUILTIN);
...
SDValue Y86TargetLowering::LowerOperation(SDValue Op,
...
bool Y86TargetLowering::shouldAvoidTransformToShift(EVT VT,
                                                       unsigned Amount) const {
  return !(Amount == 8 || Amount == 9 || Amount<=2);
}

// Implemented to verify test case assertions in
// tests/codegen/y86/shift-amount-threshold-b.ll
bool Y86TargetLowering::isLegalICmpImmediate(int64_t Immed) const {
  if (Y86NoLegalImmediate)
    return Immed >= -32 && Immed < 32;
  return TargetLowering::isLegalICmpImmediate(Immed);
}

//===----------------------------------------------------------------------===//
//                       Y86 Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TargetLowering::ConstraintType
Y86TargetLowering::getConstraintType(StringRef Constraint) const {
...
Y86TargetLowering::getRegForInlineAsmConstraint(
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':   // GENERAL_REGS
      if (VT == MVT::i8)
        return std::make_pair(0U, &Y86::GR8RegClass);

      return std::make_pair(0U, &Y86::GR16RegClass);
    }
  }
...
static void AnalyzeVarArgs(CCState &State,
  ...
  State.AnalyzeCallOperands(Outs, CC_Y86_AssignStack);
}

static void AnalyzeVarArgs(CCState &State,
                           const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeFormalArguments(Ins, CC_Y86_AssignStack);
}
...
static void AnalyzeArguments(CCState &State,
  ...
  bool Builtin = (State.getCallingConv() == CallingConv::Y86_BUILTIN);
  if (Builtin) {
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UsedStack && Parts == 2 && RegsLeft == 1) {
      ...
      UsedStack = true;
      CC_Y86_AssignStack(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
...
static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeCallResult(Ins, RetCC_Y86);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs) {
  State.AnalyzeReturn(Outs, RetCC_Y86);
}
...
SDValue
Y86TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Y86 target does not yet support tail call optimization.
  isTailCall = false;

  switch (CallConv) {
  default:
    report_fatal_error("Unsupported calling convention");
  case CallingConv::Y86_BUILTIN:
  case CallingConv::Fast:
  case CallingConv::C:
...
SDValue Y86TargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i16:
        Register VReg = RegInfo.createVirtualRegister(&Y86::GR16RegClass);
...
bool
Y86TargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                     MachineFunction &MF,
                                     bool IsVarArg,
                                     const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  return CCInfo.CheckReturn(Outs, RetCC_Y86);
}
...
Y86TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  if (CallConv == CallingConv::Y86_INTR && !Outs.empty())
  ...
  unsigned Opc = (CallConv == CallingConv::Y86_INTR ?
                  Y86ISD::RETI_FLAG : Y86ISD::RET_FLAG);
...
SDValue Y86TargetLowering::LowerCCCCallTo(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, dl, Y86::SP, PtrVT);
  ...
  Chain = DAG.getNode(Y86ISD::CALL, dl, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
///
SDValue Y86TargetLowering::LowerCallResult(
...
SDValue Y86TargetLowering::LowerShifts(SDValue Op,
                                          SelectionDAG &DAG) const {
  ...
  if (Opc == ISD::SRL && ShiftAmount) {
    ...
    // srl A, 1 => clrc; rrc A
    Victim = DAG.getNode(Y86ISD::RRCL, dl, VT, Victim);
    ShiftAmount -= 1;
  }

  while (ShiftAmount--)
    Victim = DAG.getNode((Opc == ISD::SHL ? Y86ISD::RLA : Y86ISD::RRA),
                         dl, VT, Victim);

  return Victim;
}

SDValue Y86TargetLowering::LowerGlobalAddress(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  return DAG.getNode(Y86ISD::Wrapper, SDLoc(Op), PtrVT, Result);
}

SDValue Y86TargetLowering::LowerExternalSymbol(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc dl(Op);
  ...
  return DAG.getNode(Y86ISD::Wrapper, dl, PtrVT, Result);
}

SDValue Y86TargetLowering::LowerBlockAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc dl(Op);
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  Y86CC::CondCodes TCC = Y86CC::COND_INVALID;
  switch (CC) {
  default: llvm_unreachable("Invalid integer condition!");
  case ISD::SETEQ:
    TCC = Y86CC::COND_E;     // aka COND_Z
  ...
  return DAG.getNode(Y86ISD::CMP, dl, MVT::Glue, LHS, RHS);
}


SDValue Y86TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(Y86ISD::BR_CC, dl, Op.getValueType(),
                     Chain, Dest, TargetCC, Flag);
}

SDValue Y86TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (Convert) {
    SDValue SR = DAG.getCopyFromReg(DAG.getEntryNode(), dl, Y86::SR,
                                    MVT::i16, Flag);
    if (Shift)
    ...
    return DAG.getNode(Y86ISD::SELECT_CC, dl, Op.getValueType(), Ops);
  }
}

SDValue Y86TargetLowering::LowerSELECT_CC(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDValue LHS    = Op.getOperand(0);
  SDValue RHS    = Op.getOperand(1);
...
SDValue Y86TargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue
Y86TargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  Y86MachineFunctionInfo *FuncInfo = MF.getInfo<Y86MachineFunctionInfo>();
...
SDValue Y86TargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue Y86TargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue Y86TargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  Y86MachineFunctionInfo *FuncInfo = MF.getInfo<Y86MachineFunctionInfo>();
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue Y86TargetLowering::LowerJumpTable(SDValue Op,
                                             SelectionDAG &DAG) const {
    JumpTableSDNode *JT = cast<JumpTableSDNode>(Op);
    auto PtrVT = getPointerTy(DAG.getDataLayout());
    SDValue Result = DAG.getTargetJumpTable(JT->getIndex(), PtrVT);
    return DAG.getNode(Y86ISD::Wrapper, SDLoc(JT), PtrVT, Result);
}

/// getPostIndexedAddressParts - returns true by value, base pointer and
/// offset pointer and addressing mode by reference if this node can be
/// combined with a load / store to form a post-indexed load / store.
bool Y86TargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                      SDValue &Base,
                                                      SDValue &Offset,
...
const char *Y86TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((Y86ISD::NodeType)Opcode) {
  case Y86ISD::FIRST_NUMBER:       break;
  case Y86ISD::RET_FLAG:           return "Y86ISD::RET_FLAG";
  case Y86ISD::RETI_FLAG:          return "Y86ISD::RETI_FLAG";
  case Y86ISD::RRA:                return "Y86ISD::RRA";
  case Y86ISD::RLA:                return "Y86ISD::RLA";
  case Y86ISD::RRC:                return "Y86ISD::RRC";
  case Y86ISD::RRCL:               return "Y86ISD::RRCL";
  case Y86ISD::CALL:               return "Y86ISD::CALL";
  case Y86ISD::Wrapper:            return "Y86ISD::Wrapper";
  case Y86ISD::BR_CC:              return "Y86ISD::BR_CC";
  case Y86ISD::CMP:                return "Y86ISD::CMP";
  case Y86ISD::SETCC:              return "Y86ISD::SETCC";
  case Y86ISD::SELECT_CC:          return "Y86ISD::SELECT_CC";
  case Y86ISD::DADD:               return "Y86ISD::DADD";
  }
  return nullptr;
}

bool Y86TargetLowering::isTruncateFree(Type *Ty1,
                                          Type *Ty2) const {
...
bool Y86TargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool Y86TargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // Y86 implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool Y86TargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // Y86 implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && VT1 == MVT::i8 && VT2 == MVT::i16;
}

bool Y86TargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
MachineBasicBlock *
Y86TargetLowering::EmitShiftInstr(MachineInstr &MI,
  ...
  case Y86::Rrcl8:
  case Y86::Rrcl16: {
    BuildMI(*BB, MI, dl, TII.get(Y86::BIC16rc), Y86::SR)
      .addReg(Y86::SR).addImm(1);
    Register SrcReg = MI.getOperand(1).getReg();
    Register DstReg = MI.getOperand(0).getReg();
    unsigned RrcOpc = MI.getOpcode() == Y86::Rrcl16
                    ? Y86::RRC16r : Y86::RRC8r;
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&Y86::GR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&Y86::GR8RegClass);
  ...
  // cmp 0, N
  // je RemBB
  BuildMI(BB, dl, TII.get(Y86::CMP8ri))
    .addReg(ShiftAmtSrcReg).addImm(0);
  BuildMI(BB, dl, TII.get(Y86::JCC))
    .addMBB(RemBB)
    .addImm(Y86CC::COND_E);
  ...
  BuildMI(LoopBB, dl, TII.get(Y86::PHI), ShiftReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(Y86::PHI), ShiftAmtReg)
    .addReg(ShiftAmtSrcReg).addMBB(BB)
    .addReg(ShiftAmtReg2).addMBB(LoopBB);
  if (ClearCarry)
    BuildMI(LoopBB, dl, TII.get(Y86::BIC16rc), Y86::SR)
      .addReg(Y86::SR).addImm(1);
  if (Opc == Y86::ADD8rr || Opc == Y86::ADD16rr)
    ...
    BuildMI(LoopBB, dl, TII.get(Opc), ShiftReg2)
      .addReg(ShiftReg);
  BuildMI(LoopBB, dl, TII.get(Y86::SUB8ri), ShiftAmtReg2)
    .addReg(ShiftAmtReg).addImm(1);
  BuildMI(LoopBB, dl, TII.get(Y86::JCC))
    .addMBB(LoopBB)
    .addImm(Y86CC::COND_NE);

  // RemBB:
  // DestReg = phi [%SrcReg, BB], [%ShiftReg, LoopBB]
  BuildMI(*RemBB, RemBB->begin(), dl, TII.get(Y86::PHI), DstReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
...
MachineBasicBlock *
Y86TargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                                  MachineBasicBlock *BB) const {
  unsigned Opc = MI.getOpcode();

  if (Opc == Y86::Shl8  || Opc == Y86::Shl16 ||
      Opc == Y86::Sra8  || Opc == Y86::Sra16 ||
      Opc == Y86::Srl8  || Opc == Y86::Srl16 ||
      Opc == Y86::Rrcl8 || Opc == Y86::Rrcl16)
    return EmitShiftInstr(MI, BB);

  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();

  assert((Opc == Y86::Select16 || Opc == Y86::Select8) &&
  ...
  //  ...
  BB = copy1MBB;
  BuildMI(*BB, BB->begin(), dl, TII.get(Y86::PHI), MI.getOperand(0).getReg())
