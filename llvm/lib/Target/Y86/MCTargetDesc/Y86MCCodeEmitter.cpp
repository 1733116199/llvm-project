//===-- Y86MCCodeEmitter.cpp - Convert Y86 code to machine code -----===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the Y86MCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
public:
  Y86MCCodeEmitter(MCContext &ctx, MCInstrInfo const &MCII)
      : Ctx(ctx), MCII(MCII) {}
...
unsigned Y86MCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                                const MCOperand &MO,
                                                SmallVectorImpl<MCFixup> &Fixups,
                                                const MCSubtargetInfo &STI) const {
  if (MO.isReg())
  ...
  Fixups.push_back(MCFixup::create(Offset, MO.getExpr(),
      static_cast<MCFixupKind>(Y86::fixup_16_byte), MI.getLoc()));
  Offset += 2;
  return 0;
}

unsigned Y86MCCodeEmitter::getMemOpValue(const MCInst &MI, unsigned Op,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  ...
  Y86::Fixups FixupKind;
  switch (Reg) {
  case 0:
    FixupKind = Y86::fixup_16_pcrel_byte;
    break;
  case 2:
    FixupKind = Y86::fixup_16_byte;
    break;
  default:
    FixupKind = Y86::fixup_16_byte;
    break;
...
unsigned Y86MCCodeEmitter::getPCRelImmOpValue(const MCInst &MI, unsigned Op,
                                                 SmallVectorImpl<MCFixup> &Fixups,
                                                 const MCSubtargetInfo &STI) const {
  ...
  Fixups.push_back(MCFixup::create(0, MO.getExpr(),
    static_cast<MCFixupKind>(Y86::fixup_10_pcrel), MI.getLoc()));
  return 0;
}

unsigned Y86MCCodeEmitter::getCGImmOpValue(const MCInst &MI, unsigned Op,
                                              SmallVectorImpl<MCFixup> &Fixups,
                                              const MCSubtargetInfo &STI) const {
  const MCOperand &MO = MI.getOperand(Op);
...
unsigned Y86MCCodeEmitter::getCCOpValue(const MCInst &MI, unsigned Op,
                                           SmallVectorImpl<MCFixup> &Fixups,
                                           const MCSubtargetInfo &STI) const {
  const MCOperand &MO = MI.getOperand(Op);
  assert(MO.isImm() && "Immediate operand expected");
  switch (MO.getImm()) {
  case Y86CC::COND_NE: return 0;
  case Y86CC::COND_E:  return 1;
  case Y86CC::COND_LO: return 2;
  case Y86CC::COND_HS: return 3;
  case Y86CC::COND_N:  return 4;
  case Y86CC::COND_GE: return 5;
  case Y86CC::COND_L:  return 6;
  default:
...
MCCodeEmitter *createY86MCCodeEmitter(const MCInstrInfo &MCII,
                                         ...
                                         MCContext &Ctx) {
  return new Y86MCCodeEmitter(Ctx, MCII);
}

#include "Y86GenMCCodeEmitter.inc"
