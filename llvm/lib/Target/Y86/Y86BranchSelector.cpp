//===-- Y86BranchSelector.cpp - Emit long conditional branches ---------===//
//
...
#define DEBUG_TYPE "y86-branch-select"

static cl::opt<bool>
    BranchSelectEnabled("y86-branch-select", cl::Hidden, cl::init(true),
...
namespace {
class Y86BSel : public MachineFunctionPass {
...
public:
  static char ID;
  Y86BSel() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return "Y86 Branch Selector"; }
};
char Y86BSel::ID = 0;
}
...
unsigned Y86BSel::measureFunction(OffsetVector &BlockOffsets,
...
bool Y86BSel::expandBranches(OffsetVector &BlockOffsets) {
  ...
  for (auto MBB = MF->begin(), E = MF->end(); MBB != E; ++MBB) {
    ...
    for (auto MI = MBB->begin(), EE = MBB->end(); MI != EE; ++MI) {
      ...
      if (MI->getOpcode() != Y86::JCC && MI->getOpcode() != Y86::JMP) {
        continue;
      }
      ...
      if (MI->getOpcode() == Y86::JCC && std::next(MI) != EE) {
      ...
      MI = BuildMI(*MBB, MI, dl, TII->get(Y86::Bi)).addMBB(DestBB);
      InstrSizeDiff += TII->getInstSizeInBytes(*MI);
...
bool Y86BSel::runOnMachineFunction(MachineFunction &mf) {
  MF = &mf;
  TII = static_cast<const Y86InstrInfo *>(MF->getSubtarget().getInstrInfo());
...
FunctionPass *llvm::createY86BranchSelectionPass() {
  return new Y86BSel();
}
