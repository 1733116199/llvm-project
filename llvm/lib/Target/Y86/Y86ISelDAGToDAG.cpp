...
namespace {
  ...
  public:
    Y86DAGToDAGISel(Y86TargetMachine &TM, CodeGenOpt::Level OptLevel)
  ...
  private:
    ...
    bool MatchAddress(SDValue N, Y86ISelAddressMode &AM);
    bool MatchWrapper(SDValue N, Y86ISelAddressMode &AM);
    bool MatchAddressBase(SDValue N, Y86ISelAddressMode &AM);
...
/// MatchWrapper - Try to match Y86ISD::Wrapper node into an addressing mode.
/// These wrap things that will resolve down into a symbol reference.  If no
/// match is possible, this returns true, otherwise it returns false.
bool Y86DAGToDAGISel::MatchWrapper(SDValue N, Y86ISelAddressMode &AM) {
...
bool Y86DAGToDAGISel::MatchAddressBase(SDValue N, Y86ISelAddressMode &AM) {
  // Is the base register already occupied?
  if (AM.BaseType != Y86ISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = Y86ISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
}

bool Y86DAGToDAGISel::MatchAddress(SDValue N, Y86ISelAddressMode &AM) {
  ...
  case ISD::FrameIndex:
    if (AM.BaseType == Y86ISelAddressMode::RegBase
        && AM.Base.Reg.getNode() == nullptr) {
      AM.BaseType = Y86ISelAddressMode::FrameIndexBase;
...
/// match by reference.
bool Y86DAGToDAGISel::SelectAddr(SDValue N,
                                    SDValue &Base, SDValue &Disp) {
  Y86ISelAddressMode AM;
  ...
  if (AM.BaseType == Y86ISelAddressMode::RegBase)
    if (!AM.Base.Reg.getNode())
      AM.Base.Reg = CurDAG->getRegister(Y86::SR, MVT::i16);

  Base = (AM.BaseType == Y86ISelAddressMode::FrameIndexBase)
...
bool Y86DAGToDAGISel::tryIndexedLoad(SDNode *N) {
...
bool Y86DAGToDAGISel::tryIndexedBinOp(SDNode *Op, SDValue N1, SDValue N2,
...
void Y86DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse()) {
      CurDAG->SelectNodeTo(Node, Y86::ADDframe, MVT::i16, TFI,
                           CurDAG->getTargetConstant(0, dl, MVT::i16));
      return;
    }
    ReplaceNode(Node, CurDAG->getMachineNode(
                          Y86::ADDframe, dl, MVT::i16, TFI,
                          CurDAG->getTargetConstant(0, dl, MVT::i16)));
    return;
  }
  case ISD::LOAD:
