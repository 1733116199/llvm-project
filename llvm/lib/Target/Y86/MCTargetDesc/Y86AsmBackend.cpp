//===-- Y86AsmBackend.cpp - Y86 Assembler Backend -------------------===//
//
...
public:
  Y86AsmBackend(const MCSubtargetInfo &STI, uint8_t OSABI)
      : MCAsmBackend(support::little), OSABI(OSABI) {}
  ~Y86AsmBackend() override {}
  ...
  createObjectTargetWriter() const override {
    return createY86ELFObjectWriter(OSABI);
  }
  ...
  const MCFixupKindInfo &getFixupKindInfo(MCFixupKind Kind) const override {
    const static MCFixupKindInfo Infos[Y86::NumTargetFixupKinds] = {
      // This table must be in the same order of enum in Y86FixupKinds.h.
      //
    ...
    static_assert((array_lengthof(Infos)) == Y86::NumTargetFixupKinds,
...
uint64_t Y86AsmBackend::adjustFixupValue(const MCFixup &Fixup,
                                            uint64_t Value,
                                            MCContext &Ctx) const {
  unsigned Kind = Fixup.getKind();
  switch (Kind) {
  case Y86::fixup_10_pcrel: {
    if (Value & 0x1)
...
bool Y86AsmBackend::writeNopData(raw_ostream &OS, uint64_t Count) const {
  if ((Count % 2) != 0)
    return false;

  // The canonical nop on Y86 is mov #0, r3
...
MCAsmBackend *llvm::createY86MCAsmBackend(const Target &T,
                                             const MCSubtargetInfo &STI,
                                             const MCRegisterInfo &MRI,
                                             const MCTargetOptions &Options) {
  return new Y86AsmBackend(STI, ELF::ELFOSABI_STANDALONE);
}
